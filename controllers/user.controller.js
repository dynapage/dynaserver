...
exports.loginInternal = async (req, res) => {
  const { username, password, usersite } = req.body;
  console.log('username ', username, password, usersite )
  try {
    const user = await User.findOne({ username, usersite }).select('password username');
    if (!user) {
      return res.status(401).json({
        errors: [
          {
...
exports.deleteInternalUser = async (req, res) => {
  ...
  console.log('username ', username, password, usersite )
  try {
    const user = await User.deleteOne({ username, usersite });
    if (!user) {
      return res.status(401).json({
        errors: [
          {
